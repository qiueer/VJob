...
@register.filter
def bootstrap(element, cols=2):
    markup_classes = {'label': '', 'value': '', 'single_value': ''}
    return render(element, markup_classes, cols=cols)


@register.filter
def bootstrap_inline(element, cols=2):
...
def render(element, markup_classes, cols=2):
    ...
    if element_type == 'boundfield':
        ...
        if has_management:
            ...
            context = {'formset': element, 'classes': markup_classes, "cols": cols}
        else:
            for field in element.visible_fields():
                add_input_classes(field)

            template = get_template("bootstrapform/form.html")
            context = {'form': element, 'classes': markup_classes, "cols": cols}
